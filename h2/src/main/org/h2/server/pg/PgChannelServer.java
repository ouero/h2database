 ...
 */
public class PgChannelServer implements Service {
    ...
    private int port = PgChannelServer.DEFAULT_PORT;
    private boolean portIsSet;
    private boolean stop;
    private boolean trace;
    private ServerSocketChannel serverSocketChannel;
    private final Set<PgServerChannelThread> running = Collections.
     ...
     */
    synchronized void remove(PgServerChannelThread t) {
        running.remove(t);
    }
    ...
    private boolean allow(SocketChannel socketChannel) {
        if (allowOthers) {
            return true;
        }
        try {
            return NetUtils.isLocalAddress(((InetSocketAddress) socketChannel.getRemoteAddress()).getAddress());
        ...
        try {
            while (!stop) {
                ...
                if (!allow(s)) {
                    ...
                    NetUtils2.setTcpQuickack(null, true);
                    PgServerChannelThread c = new PgServerChannelThread(s, this);
                    running.add(c);
    ...
    public void stop() {
        ...
        for (PgServerChannelThread c : new ArrayList<>(running)) {
            c.close();
            try {
    ...
    public boolean isRunning(boolean traceError) {
        ...
        try {
            Socket s = NetUtils.createLoopbackSocket(serverSocketChannel.socket().getLocalPort(), false);
            s.close();
            return true;
        } catch (Exception e) {
            if (traceError) {
     ...
     * @return the thread
     */
    PgServerChannelThread getThread(int processId) {
        for (PgServerChannelThread c : new ArrayList<>(running)) {
